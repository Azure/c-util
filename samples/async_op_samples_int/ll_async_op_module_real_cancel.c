...
typedef struct LL_ASYNC_OP_MODULE_REAL_CANCEL_RESULT_SETTINGS_TAG
{
    DLIST_ENTRY dlist_entry;
    bool completes_synchronously;
    COMMON_ASYNC_OP_MODULE_RESULT result;
} LL_ASYNC_OP_MODULE_REAL_CANCEL_RESULT_SETTINGS;
...
IMPLEMENT_MOCKABLE_FUNCTION(, void, ll_async_op_module_real_cancel_destroy, LL_ASYNC_OP_MODULE_REAL_CANCEL_HANDLE, handle)
{
    ...
    else
    {
        ...
        {
            LL_ASYNC_OP_MODULE_REAL_CANCEL_RESULT_SETTINGS* settings = CONTAINING_RECORD(entry, LL_ASYNC_OP_MODULE_REAL_CANCEL_RESULT_SETTINGS, dlist_entry);
            free(settings);
        }
...
IMPLEMENT_MOCKABLE_FUNCTION(, int, ll_async_op_module_real_cancel_add_result_settings_to_queue, LL_ASYNC_OP_MODULE_REAL_CANCEL_HANDLE, handle, bool, is_synchronous, COMMON_ASYNC_OP_MODULE_RESULT, next_result)
{
    ...
    if (handle == NULL)
    {
        LogError("Invalid arguments: LL_ASYNC_OP_MODULE_REAL_CANCEL_HANDLE handle=%p, bool is_synchronous=%" PRI_BOOL ", COMMON_ASYNC_OP_MODULE_RESULT next_result=%" PRI_MU_ENUM "",
            handle, MU_BOOL_VALUE(is_synchronous), MU_ENUM_VALUE(COMMON_ASYNC_OP_MODULE_RESULT, next_result));
        result = MU_FAILURE;
    }
    else
    {
        LL_ASYNC_OP_MODULE_REAL_CANCEL_RESULT_SETTINGS* settings = malloc(sizeof(LL_ASYNC_OP_MODULE_REAL_CANCEL_RESULT_SETTINGS));
        if (settings == NULL)
        {
            LogError("malloc LL_ASYNC_OP_MODULE_REAL_CANCEL_RESULT_SETTINGS failed");
            result = MU_FAILURE;
        }
        else
        {
