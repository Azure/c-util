        ...
        if (isHexDigit(replace))
        {
            ...
            for (size_t pos = 0; pos < UUID_T_STRING_LENGTH; pos++) /*trying to replace the character at position "pos" with "replace"*/
            {
                if (SOURCE[pos] == '-')
                {
                    ...
                    ///act
                    UUID_FROM_STRING_RESULT result = uuid_from_string(source, &destination);

                    ///assert
                    ASSERT_ARE_EQUAL(UUID_FROM_STRING_RESULT, UUID_FROM_STRING_RESULT_INVALID_DATA, result);
                    ASSERT_ARE_EQUAL(char_ptr, umock_c_get_expected_calls(), umock_c_get_actual_calls());
                }
            }
        }
