...
THANDLE_TYPE_DEFINE_WITH_MALLOC_FUNCTIONS(G_ON_T_OFF_DUMMY, global_malloc, global_malloc_flex, global_free);
...
{
    G_ON_T_OFF_DUMMY* d = THANDLE_MALLOC_WITH_EXTRA_SIZE_WITH_MALLOC_FUNCTIONS(G_ON_T_OFF_DUMMY)(NULL, strlen(s) + 1, var_malloc_flex, var_free);
    if (d != NULL)
    {
        d->x = x;
        d->n = (uint32_t)strlen(s);
