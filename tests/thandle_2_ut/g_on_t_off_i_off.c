...
/*uses global*/
THANDLE(G_ON_T_OFF_I_OFF_DUMMY) G_ON_T_OFF_I_OFF_create_with_extra_size(int x, const char* s)
{
    G_ON_T_OFF_I_OFF_DUMMY* d = THANDLE_MALLOC_WITH_EXTRA_SIZE(G_ON_T_OFF_I_OFF_DUMMY)(NULL, strlen(s)+1);
    if (d != NULL)
    {
        d->x = x;
...
static int copies_dummy(G_ON_T_OFF_I_OFF_DUMMY* destination, const G_ON_T_OFF_I_OFF_DUMMY* source)
{
...
THANDLE(G_ON_T_OFF_I_OFF_DUMMY) G_ON_T_OFF_I_OFF_create_from_content_flex(const G_ON_T_OFF_I_OFF_DUMMY* origin)
{
    G_ON_T_OFF_I_OFF_DUMMY* d = THANDLE_CREATE_FROM_CONTENT_FLEX(G_ON_T_OFF_I_OFF_DUMMY)(origin, NULL, copies_dummy, sizeof_dummy);
    if (d != NULL)
    {
        d->x = origin->x;
        d->n = origin->n;
