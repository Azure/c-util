...
static void expect_init_verbose(int threadpool_result)
{
    ...
    if (threadpool_result == 0) {
        STRICT_EXPECTED_CALL(threadpool_create(test_execution_engine));
    }
    else {
        STRICT_EXPECTED_CALL(threadpool_create(test_execution_engine)).SetReturn(NULL);
    }
...
/*Tests_SRS_WATCHDOG_THREADPOOL_42_001: [ If the watchdog threadpool has already been initialized then watchdog_threadpool_init shall fail and return a non-zero value. ]*/
TEST_FUNCTION(watchdog_threadpool_init_twice_fails)
{
    // arrange
    do_init();
...
/*Tests_SRS_WATCHDOG_THREADPOOL_42_007: [ If there are any other errors then watchdog_threadpool_init shall fail and return a non-zero value. ]*/
TEST_FUNCTION(watchdog_threadpool_init_fails_when_open_thread_pool_fails_async)
{
    // arrange
    expect_init_fail();
...
/*Tests_SRS_WATCHDOG_THREADPOOL_42_007: [ If there are any other errors then watchdog_threadpool_init shall fail and return a non-zero value. ]*/
TEST_FUNCTION(watchdog_threadpool_init_fails_when_underlying_functions_fail)
{
    // arrange
    expect_init();
...
/*Tests_SRS_WATCHDOG_THREADPOOL_42_009: [ If the watchdog threadpool has not been initialized then watchdog_deinit shall return. ]*/
TEST_FUNCTION(watchdog_threadpool_deinit_without_init_returns)
{
    // arrange
    STRICT_EXPECTED_CALL(interlocked_compare_exchange(IGNORED_ARG, IGNORED_ARG, IGNORED_ARG));
...
/*Tests_SRS_WATCHDOG_THREADPOOL_42_013: [ After watchdog_threadpool_deinit returns then watchdog_threadpool_init may be called again. ]*/
TEST_FUNCTION(watchdog_threadpool_init_works_after_deinit)
{
    // arrange
    do_init();
    expect_deinit();
...
/*Tests_SRS_WATCHDOG_THREADPOOL_42_014: [ watchdog_threadpool_get shall return the threadpool created by watchdog_threadpool_init. ]*/
TEST_FUNCTION(watchdog_threadpool_get_returns_threadpool)
{
    // arrange
    do_init();
...
/*Tests_SRS_WATCHDOG_THREADPOOL_42_015: [ If the watchdog threadpool has not been initialized then watchdog_threadpool_get shall return NULL. ]*/
TEST_FUNCTION(watchdog_threadpool_get_without_init_returns_NULL)
{
    // arrange
    STRICT_EXPECTED_CALL(interlocked_add(IGNORED_ARG, 0));
