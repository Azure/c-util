...
#include "c_util/channel.h"
    ...
    THANDLE(CHANNEL) real_channel_create(THANDLE(PTR(LOG_CONTEXT_HANDLE)) log_context, THANDLE(THREADPOOL) threadpool, uint32_t channel_capacity);
    ...
    uint32_t real_channel_get_count_of_items_in_channel(THANDLE(CHANNEL) channel);
    int real_channel_seal_channel(THANDLE(CHANNEL) channel);
