...
#define DEFINE_TRC_PTR_LL_DISPOSE_FUNC(C, T) \
    ...
    { \
        ...
        { \
            /* Codes_SRS_TRC_PTR_45_006: [ If free_func is not NULL, T_rc_ptr_dispose shall call free_func with the context ptr. ]*/ \
...
#define DEFINE_TRC_PTR_LL_CREATE_WITH_MOVE_POINTER(C, T) \
    ...
    { \
        ...
        if (ptr == NULL) \
        { \
            /* Codes_SRS_TRC_PTR_45_001: [ If ptr is NULL, TRC_PTR_CREATE_WITH_MOVE_POINTER(T) shall fail and return NULL. ]*/ \
            LogError("Invalid arguments: void* ptr=%p, " MU_TOSTRING(MU_C2(C, _rc_ptr_free_func)) " free_func = %p", ptr, free_func); \
        } \
        else \
        { \
            /* Codes_SRS_TRC_PTR_45_002: [ TRC_PTR_CREATE_WITH_MOVE_POINTER(T) shall create a TRC_PTR(T) by calling THANDLE_MALLOC(T) with MU_C2(C, _rc_ptr_dispose) as the dispose function. ]*/ \
            THANDLE(TRC_PTR_STRUCT(T)) thandle = THANDLE_MALLOC(TRC_PTR_STRUCT(T))(MU_C2(C, _rc_ptr_dispose)); \
            if (thandle == NULL) \
            { \
                /* Codes_SRS_TRC_PTR_45_004: [ If there are any failures, TRC_PTR_CREATE_WITH_MOVE_POINTER(T) shall fail and return NULL. ]*/ \
                LogError("Failure in THANDLE_MALLOC(" MU_TOSTRING(TRC_PTR_STRUCT(T)) ")(rc_ptr_dispose=%p)", MU_C2(C, _rc_ptr_dispose)); \
            } \
            else \
            { \
                /* Codes_SRS_TRC_PTR_45_003: [ TRC_PTR_CREATE_WITH_MOVE_POINTER(T) shall store the given ptr and free_func in the created THANDLE(TRC_PTR_STRUCT(T)). ]*/ \
                TRC_PTR_STRUCT(T)* rc_ptr = THANDLE_GET_T(TRC_PTR_STRUCT(T))(thandle); \
                rc_ptr->ptr = ptr; \
                rc_ptr->free_func = free_func; \
                /* Codes_SRS_TRC_PTR_45_005: [ TRC_PTR_CREATE_WITH_MOVE_POINTER(T) shall succeed and return a non-NULL value. ] */ \
