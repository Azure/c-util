...
#define CONSTBUFFER_ARRAY_BATCH_VERSION 1

#define CONSTBUFFER_ARRAY_BATCH_VERSION_OFFSET  0
#define CONSTBUFFER_ARRAY_BATCH_VERSION_SIZE    sizeof(uint8_t)

#define CONSTBUFFER_ARRAY_BATCH_COUNT_OFFSET    (CONSTBUFFER_ARRAY_BATCH_VERSION_OFFSET + CONSTBUFFER_ARRAY_BATCH_VERSION_SIZE)
#define CONSTBUFFER_ARRAY_BATCH_COUNT_SIZE      sizeof(uint32_t)

#define CONSTBUFFER_ARRAY_BATCH_ITEM_BUFFER_COUNT_START (CONSTBUFFER_ARRAY_BATCH_COUNT_OFFSET + CONSTBUFFER_ARRAY_BATCH_COUNT_SIZE)
...
CONSTBUFFER_ARRAY_HANDLE constbuffer_array_batcher_batch(CONSTBUFFER_ARRAY_HANDLE* payloads, uint32_t count)
{
    ...
    else
    {
        ...
        else
        {
            ...
            /* Codes_SRS_CONSTBUFFER_ARRAY_BATCHER_01_004: [ constbuffer_array_batcher_batch shall allocate memory for the header buffer (enough to hold the entire batch header: (count + 1) uint32_t values) + 1 byte for the version. ]*/
            size_t needed_memory = sizeof(uint32_t) * (count + 1) + 1;
            header_memory = malloc(needed_memory);
            if (header_memory == NULL)
            {
                /* Codes_SRS_CONSTBUFFER_ARRAY_BATCHER_01_010: [ If any error occurs, constbuffer_array_batcher_batch shall fail and return NULL. ]*/
            ...
            else
            {
                ...
                /* Codes_SRS_CONSTBUFFER_ARRAY_BATCHER_01_027: [ The version (1) shall be written as the first byte of the header. ]*/
                write_uint8_t(header_memory + CONSTBUFFER_ARRAY_BATCH_VERSION_OFFSET, CONSTBUFFER_ARRAY_BATCH_VERSION);

                /* Codes_SRS_CONSTBUFFER_ARRAY_BATCHER_01_005: [ count shall be written as a uint32_t in the header memory per the above layout. ]*/
                write_uint32_t(header_memory + CONSTBUFFER_ARRAY_BATCH_COUNT_OFFSET, count);
                ...
                for (i = 0; i < count; i++)
                {
                    ...
                    /* Codes_SRS_CONSTBUFFER_ARRAY_BATCHER_01_006: [ The count of buffers for each array in payloads shall also be written in the header per the above layout. ]*/
                    write_uint32_t(header_memory + CONSTBUFFER_ARRAY_BATCH_ITEM_BUFFER_COUNT_START + (i * sizeof(uint32_t)), buffer_count);
                ...
                needed_memory = sizeof(CONSTBUFFER_HANDLE) * (total_buffer_count + 1);
                all_buffers = malloc(needed_memory);
                if (all_buffers == NULL)
                {
                    /* Codes_SRS_CONSTBUFFER_ARRAY_BATCHER_01_010: [ If any error occurs, constbuffer_array_batcher_batch shall fail and return NULL. ]*/
                    LogError("malloc(%zu) failed", needed_memory);
                ...
                else
                {
                    ...
                    all_buffers[current_index] = CONSTBUFFER_CreateWithMoveMemory(header_memory, sizeof(uint32_t) * (count) + CONSTBUFFER_ARRAY_BATCH_ITEM_BUFFER_COUNT_START);
...
CONSTBUFFER_ARRAY_HANDLE* constbuffer_array_batcher_unbatch(CONSTBUFFER_ARRAY_HANDLE batch, uint32_t* payload_count)
{
    ...
    else
    {
        ...
        const uint8_t* header_buffer_memory;
        ...
        else
        {
            ...
            if (header_buffer_content->size < CONSTBUFFER_ARRAY_BATCH_ITEM_BUFFER_COUNT_START)
            {
                /* Codes_SRS_CONSTBUFFER_ARRAY_BATCHER_01_024: [ If the size of the first buffer is less than 5, constbuffer_array_batcher_unbatch shall fail and return NULL. ]*/
            ...
            else
            {
                ...
                /* Codes_SRS_CONSTBUFFER_ARRAY_BATCHER_01_028: [ constbuffer_array_batcher_unbatch shall read the version byte from the header. ]*/
                read_uint8_t(header_buffer_memory + CONSTBUFFER_ARRAY_BATCH_VERSION_OFFSET, &batch_version);

                if (batch_version != CONSTBUFFER_ARRAY_BATCH_VERSION)
                {
                    /* Codes_SRS_CONSTBUFFER_ARRAY_BATCHER_01_029: [ If the version is not 1, constbuffer_array_batcher_unbatch shall fail and return NULL. ]*/
                    LogError("Unknown version: %" PRIu8 ", expected: %" PRIu8 "", batch_version, CONSTBUFFER_ARRAY_BATCH_VERSION);
                }
                else
                {
                    ...
                    read_uint32_t(header_buffer_memory + CONSTBUFFER_ARRAY_BATCH_COUNT_OFFSET, &batch_payload_count);
                    ...
                    if (batch_payload_count == 0)
                    {
                        ...
                        ((header_buffer_content->size - CONSTBUFFER_ARRAY_BATCH_ITEM_BUFFER_COUNT_START) / sizeof(uint32_t) != batch_payload_count) ||
                        ((header_buffer_content->size - CONSTBUFFER_ARRAY_BATCH_ITEM_BUFFER_COUNT_START) % sizeof(uint32_t) != 0)
                        )
                    {
                    ...
                    {
                        /* Codes_SRS_CONSTBUFFER_ARRAY_BATCHER_01_017: [ constbuffer_array_batcher_unbatch shall allocate enough memory to hold the handles for buffer arrays that will be un-batched. ]*/
                        size_t needed_memory = sizeof(CONSTBUFFER_ARRAY_HANDLE) * batch_payload_count;
                        ...
                        else
                        {
                            ...
                            for (i = 0; i < batch_payload_count; i++)
                            {
                                ...
                                read_uint32_t(header_buffer_memory + CONSTBUFFER_ARRAY_BATCH_ITEM_BUFFER_COUNT_START + (i * sizeof(uint32_t)), &buffer_count);
